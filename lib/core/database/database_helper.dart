...
class DatabaseHelper {
  ...
  Future<void> _insertHsCodeDefaultData(Database db) async {
    ...
    final defaultCategories = [
      {
        'category_code': '8471', 'category_name': 'أجهزة كمبيوتر وملحقاتها',
        ...
        'fixed_fees': [{'code': 'SEAL', 'name': 'رسوم السيل', 'amount': 5000}],
      },
      {
        'category_code': '2402', 'category_name': 'سجائر ومنتجات تبغ',
        'description': 'سيجار وسيجاريللو وسجائر من تبغ',
        'relative_fees': [
      ...
      {
        'category_code': '8703', 'category_name': 'سيارات ومركبات',
        'description': 'سيارات ركوب وأنواع أخرى',
        'relative_fees': [
      ...
      {
        'category_code': '1006', 'category_name': 'أرز',
      ...
      {
        'category_code': '0402', 'category_name': 'حليب ومشتقاته',
  ...
  Future<List<Map<String, dynamic>>> getTariffCategories() async {
    final db = await database;
    return await db.rawQuery('SELECT DISTINCT category FROM tariff_items ORDER BY category ASC');
  }

  Future<List<Map<String, dynamic>>> getTariffByCategory(String category) async {
    final db = await database;
    return await db.query('tariff_items', where: 'category = ?', whereArgs: [category], orderBy: 'name ASC');
  }

  Future<Map<String, dynamic>?> getTariffByCode(String code) async {
    final db = await database;
    final results = await db.query('tariff_items', where: 'code = ?', whereArgs: [code], limit: 1);
